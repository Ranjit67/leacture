...
class Dashboarded extends Component {
  ...
  playlistCreate = async () => {
    ...
    await fetch("https://sqlnbackend.herokuapp.com/createplaylist", {
      method: "POST",
      ...
      .then((data) => {
        if (
          data.error &&
          (data.error.status === 405 || data.error.status === 401)
        ) {
          this.refresh(2);
        }
        ...
        if (data.savePlaylist) {
          ...
          this.setState({ imageUrl: { value: "", bolean: false } });
          this.setState({ playlistName: "", localStoreImageName: "" });
        }
      })
      .catch((err) => console.log(err));
  };
  ...
  dashboard = async () => {
    ...
    //https://leacteer-api.herokuapp.com/
    await fetch("https://sqlnbackend.herokuapp.com/dashboard", {
      method: "POST",
  ...
  imageUpload = async (e) => {
    ...
    await fetch("https://api.cloudinary.com/v1_1/dhlqc1hup/image/upload", {
      ...
      .then((data) => {
        ...
        this.setState({ imageUrl: { value: data.secure_url, bolean: false } });
      })
      .catch((err) => console.log(err));
  };
          ...
          >
            ...
            <div className="modelItem">
              {this.state.emptyText && (
                <h3 className="warnMassage">You have to fill all the field.</h3>
              )}
              {this.state.sameTtitle && (
                <h3 className="warnMassage">This title is already exit.</h3>
              )}
              ...
              {this.state.imageUrl.bolean && (
                <div>
                  <h2>Loading....</h2>
                  <p className="warnUplodepicforButton">
                    Keep wait stile upload the THOAMNAIL picture
                  </p>
                </div>
              )}
